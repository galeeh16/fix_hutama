...
class Penyakit extends CI_Controller {
	...
	public function get_data()
	{
		...
		$count = $this->Model_penyakit->get_data_count($search);

		$limit = ceil($count / $rowsperpage);

		$data_penyakit = $this->Model_penyakit->get_data($p, $rowsperpage, $search);
		...
		if($count > 0) {
			$no = $p + 1;
			foreach ($data_penyakit as $key => $row) {
				$html .= '<tr>';
				$html .= '<td>' . $no . '</td>';
				$html .= '<td>' . $row->kode_penyakit . '</td>';
				$html .= '<td>' . $row->nama_penyakit . '</td>';
				$html .= '<td class="text-justify">' . $row->deskripsi_penyakit . '</td>';
				$html .= '<td class="text-justify">' . $row->nama_solusi . '</td>';
				$html .= '<td>' . '<img src="'.base_url().'assets/img/penyakit/'.$row->foto.'" class="img-thumbnail" />' . '</td>';

				$html .= '<td class="text-center"><button type="button" title="Ubah User" onclick="detail('."'$row->id_penyakit'".')" class="btn btn-info btn-xs"><i class="lnr lnr-eye"></i></button></td>';

				$html .= '<td class="text-center"><button type="button" title="Ubah User" onclick="submit('."'$row->id_penyakit'".')" class="btn btn-warning btn-xs"><i class="lnr lnr-pencil"></i></button></td>';

				$html .= '<td class="text-center"><button type="button"onclick="hapus('."'$row->id_penyakit'".')" title="Hapus User" class="btn btn-danger btn-xs"><i class="lnr lnr-trash"></i></button></td>';
			...
			$html .= '<tr>
				<td class="text-center" colspan="9"><div class="alert alert-info"><i class="fa fa-info-circle"></i> Data tidak ditemukan</div></td>
			</tr>';
		}
